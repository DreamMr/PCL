...
class PoseConDB(data.Dataset):
    def __init__(self, config, phase):
        super(PoseConDB, self).__init__()
    ...
    def __getitem__(self, item):
        ...
        neg_list = []
        # exp_img_crop1_flip = self.flip(exp_img_crop1)
        # exp_img_crop2_flip = self.flip(exp_img_crop2)
        img_flip = self.flip(img)
        for i in range(100):
            # if random.random()>0.5:
            #     neg_list.append(self.data_transforms(exp_img_crop1_flip))
            # else:
            #     neg_list.append(self.data_transforms(exp_img_crop2_flip))
            #neg_img1,neg_img2 = self.distance_crop(img_flip)
            neg_list.append(self.data_transforms(img_flip))
            #neg_list.append(self.data_transforms(neg_img2))
